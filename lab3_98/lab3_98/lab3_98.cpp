...
int main() {
    ...
    for (int i = 0; i < thread_emount; i++) {
        numbers[i] = i + 1;
        hThread[i] = CreateThread(nullptr, 0, (LPTHREAD_START_ROUTINE)Marker, &numbers[i], CREATE_SUSPENDED, nullptr);
        ResumeThread(hThread[i]);

        Sleep(synchronization_time);
    }

    while (!all_zero(end_of_threads, thread_emount)) {
        for (int i = 0; i < thread_emount; i++) {
