...
int main(int argc, char* argv[])
{
	...
	for (int i = 0; i < sendersEmount; i++)
	{
		...
		strcpy(data[i], SenderAllocation);
		strcat(data[i], " ");
		strcat(data[i], file_name);
		strcat(data[i], " ");
		strcat(data[i], names_senders_ready[i]);
		strcat(data[i], " ");
		strcat(data[i], names_continue_work[i]);
		strcat(data[i], " ");
		strcat(data[i], names_end_work[i]);
		strcat(data[i], " ");
		strcat(data[i], name_str_counter);
		strcat(data[i], " ");
		strcat(data[i], name_sender_counter);
		strcat(data[i], " ");
		strcat(data[i], senderNum);
	}
	...
	while (!all_zero(end_of_threads, sendersEmount))
	{
		for (int i = 0; i < sendersEmount; ++i)
		{
		...
		if (action == 1)
		{
			...
			{
				readMessage(file_bin, readed_message);
				printf("%s\n", readed_message);
			}
